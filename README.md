...
- **Taint analysis:** inter-procedural and cross-file. A source in one function reaching a sink in another — or in another file — is detected in every supported language. Data flows through arguments, return values, object fields, containers and loop bindings; call edges and declaration edges are treated as control flow, so a value in a function does not implicate everything that function invokes. Impact is classified per class (RCE, SQL/NoSQL injection, SSRF, XSS, memory corruption, path traversal, deserialization)
- **Measured, not asserted:** scored against OWASP BenchmarkJava's published ground truth — 1,572 labelled cases across six injection classes — at **65.0% recall and 50.5% precision**. Run it yourself with `tools/score_owasp_benchmark.py`. The benchmark's safe cases are built to require constant propagation, which this engine does not do, so its false-positive rate is a worst case rather than the rate on real code; both numbers and the reasons are in [docs/validation.md](docs/validation.md)
